    ...
    def forward(self, inputs,iftest=False):
        ...
        for framenum in range(self.args.seq_length-1):
            ...
            for p in range(self.args.passing_time):
                if p==0:
                    res_len = self.gcn.forward(corr_index, nei_index, nei_num_index, lstm_state,self.gcn.W_nei)
                    if  res_len is not None and len(res_len)==1 :
                        lstm_state = self.gcn.forward(corr_index, nei_index, nei_num_index, lstm_state,self.gcn.W_nei)
                else:
                    continue
                if p==1:
                    res_len = self.gcn.forward(corr_index, nei_index, nei_num_index, lstm_state,self.gcn.W_nei)
