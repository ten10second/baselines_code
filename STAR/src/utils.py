...
class Trajectory_Dataloader():
    def __init__(self, args):
        ...
        self.args = args
        if self.args.dataset == './sysu_data_star':

            entries = os.listdir(self.args.dataset)
            self.data_dirs = [os.path.join(self.args.dataset, entry) for entry in entries if os.path.isdir(os.path.join(self.args.dataset, entry))]
    ...
    def traject_preprocess(self, setname):
        ...
        for seti, directory in enumerate(data_dirs):
            ...
            for file_path in file_paths:
                current_array = np.genfromtxt(file_path, delimiter=' ')
                all_arrays.append(current_array)
            combined_array = np.vstack(all_arrays)

            # Load the data from the csv file
            data = np.transpose(combined_array)  #(4, 6154)
    ...
    def find_trajectory_fragment(self, trajectory, startframe, seq_length, skip):
        ...
        if offset_end-offset_start+1 == candidate_seq.shape[0]:
...
def L2forTestS(outputs, targets, obs_length, lossMask, num_samples=20):
    ...
    # print(error_full.shape)   # (20, 12, peds)
    if error_full.shape[2] != 0:
        error_full_sum = torch.sum(error_full, dim=1) # (20, peds)
        error_full_sum_min, min_index = torch.min(error_full_sum, dim=0)  # (peds)
        best_error = []
        ...
        error_full_sum = torch.zeros(20, 3, device='cuda:0')# torch.zeros(20, peds)
        error_full_sum_min, min_index = torch.min(error_full_sum, dim=0)  # (peds)

        error = torch.sum(error_full_sum_min)
        error_cnt = error_full.numel() / num_samples
        final_error = torch.tensor(0.0, device='cuda:0')
        final_error_cnt = error_full.shape[-1]
