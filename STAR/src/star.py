    ...
    def forward(self, inputs, iftest=False):
        ...
        for framenum in range(self.args.seq_length - 1):
            ...
            if framenum >= self.args.obs_length and iftest:
                ...
                if nodes_current.shape[1] == 0:
                    nodes_current = torch.zeros(framenum+1, 3, 2, device='cuda:0')
                    node_abs = torch.zeros(framenum+1, 3, 2, device='cuda:0')
                    nei_list = torch.eye(3, device='cuda:0')
            ...
            if framenum == 0:
                ...
                if node_index.numel() == 0:
                    zeros_tensor = torch.zeros(temporal_input_embedded[:framenum].shape, device='cuda:0')
                    # 将零张量赋值给temporal_input_embedded的前framenum个元素
                    temporal_input_embedded[:framenum] = zeros_tensor
                else:
            ...
            if node_index.numel() == 0:
                outputs[framenum] = torch.zeros_like(outputs[framenum])
                GM[framenum] = torch.zeros_like(GM[framenum])
            else:
